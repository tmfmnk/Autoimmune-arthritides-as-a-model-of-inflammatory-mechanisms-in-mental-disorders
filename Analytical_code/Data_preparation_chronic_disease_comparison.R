...
#Excluding records with hospitalization for the chronic health condition
...
#Selecting hospitalizations between 1999 and 2012 with chronic disorders, but not for the specific rheumatic disorder
...
#Establish the number of hospitalizations in the past 5 years

past_hosp <- map(cohorts_baseline_with_outcome,
                 function(data_rheuma_dg) {
                   ...
                   map(data_rheuma_dg,
                       function(data_rheuma_and_psychiatric_dg) {
                         ...
                         data_rheuma_and_psychiatric_dg %>%
                           ...
                           group_by(RODCIS2_exposed, RODCIS2) %>%
                           summarise(past_hosp_n = sum(past_hosp) - 1) %>%
                           ungroup()
                       })
                 })

#Merge number of past hospitalizations with the baseline information and outcome 

cohorts_baseline_with_outcome_and_past_hosp <- map2(cohorts_baseline_with_outcome,
                                                    past_hosp,
                                                    function(data_rheuma_dg, past_hosp_rheuma_dg) {
                                                      
                                                      map2(data_rheuma_dg,
                                                           past_hosp_rheuma_dg,
                                                           function(data_rheuma_and_psychiatric_dg, past_hosp_rheuma_and_psychiatric_dg) {
                                                             
                                                             data_rheuma_and_psychiatric_dg %>%
                                                               left_join(past_hosp_rheuma_and_psychiatric_dg,
                                                                         by = c("RODCIS2", "RODCIS2_exposed"))
...
#Sanity check
#Do past hospitalizations start from 0?

map(cohorts_baseline_with_outcome_and_past_hosp,
    function(data_rheuma_dg) {
      
      all(map_lgl(data_rheuma_dg,
                  function(data_rheuma_and_psychiatric_dg) {
                    
                    data_rheuma_and_psychiatric_dg %>%
                      summarise(cond = all(past_hosp_n >= 0)) %>%
                      pull(cond)
...
data_chronic_disease_comparison <- map(cohorts_baseline_with_outcome_and_past_hosp,
                                       function(data_rheuma_dg) {
